...
describe("EXEC_WRAPPERS", () => {
  ...
  it("covers the ruling's examples on every host and the vault's own wrapper binaries", () => {
    ...
    // The vault's own win32 wrapper: the version-suffix rule strips digits, so a
    // hash with a letter in it survives it — but a hash in the *filename* changes
    // the basename this tier matches on, and `harpoc-job-0123456789abcdef` would
    // escape EXEC_WRAPPERS altogether. That escape is why the hash sits in the
    // directory instead.
    expect(
