...
- **Audit trail** — every vault operation logged, detail fields encrypted at rest, rows HMAC-chained (`harpoc audit verify`)
...
- **Tamper-evident audit log**: detail fields are encrypted and bound to their row, and rows are HMAC-chained — `harpoc audit verify` detects modification or deletion.
